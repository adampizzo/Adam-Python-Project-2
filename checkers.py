...
def is_player_height_int(players_list, logging_enabled):
    ''' This function tests whether the clean_player_height function's
        output actually changes it to int or not.'''
...
def is_player_experience_bool(players_list, logging_enabled):
    ''' This function tests whether the clean_player_experience function's
        output actually changes it to bool or not.'''
...
def is_guardians_a_list(players_list, logging_enabled):
    ''' This function tests whether the clean_player_guardians function's
        output actually changes the value to a list or not'''
...
def write_player_list(players_list):
    ''' The purpose of the write_player_list function is to write a listed
        provided as an argument to a log text file. I wanted to test out
        writing a file instead of just logging, which this could have done
        instead.'''
...
def print_player_list(players_list):
    ''' The purpose of the print_player_list is to have a command to print out all
        players and their teams in the console. Originally used for testing
        code. The write player list became more useful, but I'll keep this
        in case it becomes handy for something in the future.'''
    ...
    for player in players_list:
        if "team" not in player.keys():
            ...
            print(player_format.format(player['name'], player['team']))


def print_team_roster(team_name, teams_list, players_list):
    ''' Takes an argument for a team name to print its roster.

        Verifies that the team being searched is in the teams_list
        otherwise prints an error.

        Utilizes the players_list list to search for who is
        on what team by each player['team'] key valuen and
        prints the players that match.'''
