...
def sleep():
    ''' The purpose of the sleep function is to cut down on me writing time.sleep(x) multiple times. If I ever need to
        change the pauses it will just be changing this function. 
    '''
    time.sleep(.5)


def get_menu_items(options_list):
    ''' The purpose of the set_menu_items function is to set the menu items for the program. If they ever need to be changed,
        this function can be called to do that. 
    '''
    return options_list


def add_dots():
    ''' The purpose of the add_dots function is to print 10 periods in a row. This function is cosmetic and is to cut down on typing this code
        over and over again. 
    '''
    for _ in range(10):
        print(".", end="")
...
def clean_data(players_list):
    ...
    sleep()
    if bool_height_cleaned and bool_experience_cleaned and bool_guardians_cleaned == True:
        add_dots()
...
def balance_teams(teams_list, players_list, balanced, randomize):
    ''' This function will add players to each team either randomly and/or assigning them in a balanced manner based off their experience value.
        By default, the options in config.py set this to balanced = True and randomize = False, however, the user can be asked to specify their
...
def get_players_by_experience(players_list):
    ''' The purpose of the get_players_by_experience function is to take a list of players and return two separate lists based on their
        experience. One list will only be for players who have True for their experience dictionary key value. The other list will
        be for players who have False for their experience dictionary key value. '''
...
def get_num_players_by_experience(team_name, players_list):
    ''' The purpose of get_num_players_by_experience function takes the team that they are on and the player list as well as if they are. 
        experienced or not. It then returns the length of a list of all players on that list.
    '''
...
def add_players_to_team(teams_list, players_list):
    ''' The purpose of the add_players_to_team function is to add players to a team by cycling through all of the available teams and
        ...
        amount of teams there are in teams_list, then it will set the counter to 0.

        This also writes the players it adds to each team to a file under the log folder, if logging is enabled.
    '''
...
def get_team_index(team_selection, teams_list):
    ''' The purpose of get_team_index function is to take a team selection from the menu selection screen and return the
        actual index of the team.
    '''
    return teams_list[int(team_selection) - 1]


def display_team_stats(team_selection, teams_list, players_list):    
    ''' The purpose of display_team_stats is to display the Team name, the total amount of players on the team, the total
        amount of expeirenced and inexperienced players on the team, the average height of the team, and the guardians of 
